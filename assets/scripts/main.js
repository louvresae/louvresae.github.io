//* ======================== Slide Control ===================== */
// Support multiple slide menus on the same page
function initializeSlideMenu(menuId) {
  var menu = document.getElementById(menuId);
  if (!menu) return;
  
  var container = menu.closest('.container');
  if (!container) return;
  
  var contents = container.getElementsByClassName("slide-content");
  
  menu.addEventListener("click", function(e) {
    const idx = [...this.children]
      .filter(el => el.className.indexOf('dot') > -1)
      .indexOf(e.target);
      
    if (idx >= 0) {
      // Remove active from all dots in this menu
      var prevDots = menu.querySelectorAll(".dot.active");
      prevDots.forEach(dot => dot.classList.remove("active"));
      e.target.classList.add("active");
      
      // Show the selected slide
      for (var i = 0; i < contents.length; i++) {
        if (i == idx) {
          contents[i].style.display = "block";
        } else {
          contents[i].style.display = "none";
        }
      }  
    }
  });
}

// Initialize all slide menus
document.addEventListener('DOMContentLoaded', function() {
  initializeSlideMenu('slide-menu');
  initializeSlideMenu('concept-slides');
  initializeSlideMenu('style-slides');
});

//* ======================== Concept Steering Control ===================== */
// Map concept names to their image paths
const conceptPaths = {
  'painterliness': {
    folder: 'assets/figures/concept-steering/painterliness/',
    files: ['0.jpg', '1.jpg', '2.jpg', '3.jpg'],
    labels: ['α₁', 'α₂', 'α₃', 'α₄']
  },
  'aerial': {
    folder: 'assets/figures/concept-steering/aerial-perspective/',
    files: ['0.jpg', '1.jpg', '2.jpg', '3.jpg'],
    labels: ['α₁', 'α₂', 'α₃', 'α₄']
  },
  'water': {
    folder: 'assets/figures/concept-steering/steering/concept_00948/',
    files: ['0.jpg', '1.jpg', '2.jpg', '3.jpg', '4.jpg'],
    labels: ['α₁', 'α₂', 'α₃', 'α₄', 'α₅']
  },
  'faceted': {
    folder: 'assets/figures/concept-steering/steering/concept_02662/',
    files: ['0.jpg', '1.jpg', '2.jpg', '3.jpg', '4.jpg'],
    labels: ['α₁', 'α₂', 'α₃', 'α₄', 'α₅']
  },
  'flat': {
    folder: 'assets/figures/concept-steering/steering/concept_03479/',
    files: ['0.jpg', '1.jpg', '2.jpg', '3.jpg', '4.jpg'],
    labels: ['α₁', 'α₂', 'α₃', 'α₄', 'α₅']
  },
  'technical': {
    folder: 'assets/figures/concept-steering/steering/concept_10979/',
    files: ['0.jpg', '1.jpg', '2.jpg', '3.jpg', '4.jpg'],
    labels: ['α₁', 'α₂', 'α₃', 'α₄', 'α₅']
  },
  'watercolor': {
    folder: 'assets/figures/concept-steering/steering/concept_18858/',
    files: ['0.jpg', '1.jpg', '2.jpg', '3.jpg', '4.jpg'],
    labels: ['α₁', 'α₂', 'α₃', 'α₄', 'α₅']
  },
  'pink': {
    folder: 'assets/figures/concept-steering/steering/concept_02458/',
    files: ['0.jpg', '1.jpg', '2.jpg', '3.jpg', '4.jpg'],
    labels: ['α₁', 'α₂', 'α₃', 'α₄', 'α₅']
  },
  'curls': {
    folder: 'assets/figures/concept-steering/steering/concept_16679/',
    files: ['0.jpg', '1.jpg', '2.jpg', '3.jpg', '4.jpg'],
    labels: ['α₁', 'α₂', 'α₃', 'α₄', 'α₅']
  }
};

function updateConceptImage(conceptName, value) {
  const concept = conceptPaths[conceptName];
  if (!concept) return;
  
  const idx = parseInt(value);
  const imagePath = concept.folder + concept.files[idx];
  
  // Update the image
  const imgElement = document.getElementById(conceptName + '-result');
  if (imgElement) {
    imgElement.src = imagePath;
  }
}

//* ======================== Video Control ===================== */
function ToggleVideo(x) {
  var videos = document.getElementsByClassName(x + '-video');
  for (var i = 0; i < videos.length; i++) {
      if (videos[i].paused) {
          videos[i].play();
      } else {
          videos[i].pause();
      }
  }
};


function SlowVideo(x) {
  var videos = document.getElementsByClassName(x + '-video');
  for (var i = 0; i < videos.length; i++) {
    videos[i].playbackRate = videos[i].playbackRate * 0.9;
    videos[i].play();
  }
  
  var msg = document.getElementById(x + '-msg');
  msg.innerHTML = 'Speed: ' + '×' + videos[0].playbackRate.toFixed(2);

  msg.classList.add("fade-in-out");
  msg.style.animation = 'none';
  msg.offsetHeight; /* trigger reflow */
  msg.style.animation = null; };


function FastVideo(x) {
  var videos = document.getElementsByClassName(x + '-video');
  for (var i = 0; i < videos.length; i++) {
    videos[i].playbackRate = videos[i].playbackRate / 0.9;
    videos[i].play();
  }

  var msg = document.getElementById(x + '-msg');
  msg.innerHTML = 'Speed: ' + '×' + videos[0].playbackRate.toFixed(2);

  msg.classList.add("fade-in-out");
  msg.style.animation = 'none';
  msg.offsetHeight; /* trigger reflow */
  msg.style.animation = null; 
};

function RestartVideo(x) {
  var videos = document.getElementsByClassName(x + '-video');
  for (var i = 0; i < videos.length; i++) {
    videos[i].pause();
    videos[i].playbackRate = 1.0;
    videos[i].currentTime = 0;
    videos[i].play();
  }
  
  var msg = document.getElementById(x + '-msg');
  msg.innerHTML = 'Speed: ' + '×' + videos[0].playbackRate.toFixed(2);

  msg.classList.add("fade-in-out");
  msg.style.animation = 'none';
  msg.offsetHeight; /* trigger reflow */
  msg.style.animation = null; 
};

//* ======================== Slide Show Control ===================== */
const slider = document.querySelector('.container .slider');
const [btnLeft, btnRight] = ['prev_btn', 'next_btn'].map(id => document.getElementById(id));
let interval;

// Set positions
const setPositions = () => 
    [...slider.children].forEach((item, i) => 
        item.style.left = `${(i-1) * 440}px`);

// Initial setup
setPositions();

// Set transition speed
const setTransitionSpeed = (speed) => {
    [...slider.children].forEach(item => 
        item.style.transitionDuration = speed);
};

// Slide functions
const next = (isAuto = false) => { 
    setTransitionSpeed(isAuto ? '1.5s' : '0.2s');
    slider.appendChild(slider.firstElementChild); 
    setPositions(); 
};

const prev = () => { 
    setTransitionSpeed('0.2s');
    slider.prepend(slider.lastElementChild); 
    setPositions(); 
};

// Auto slide
const startAuto = () => interval = interval || setInterval(() => next(true), 2000);
const stopAuto = () => { clearInterval(interval); interval = null; };

// Event listeners
btnRight.addEventListener('click', () => next(false));
btnLeft.addEventListener('click', prev);

// Mouse hover controls
[slider, btnLeft, btnRight].forEach(el => {
    el.addEventListener('mouseover', stopAuto);
    el.addEventListener('mouseout', startAuto);
});

// Start auto slide
startAuto();